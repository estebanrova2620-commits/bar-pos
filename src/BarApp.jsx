...
export default function BarApp() {
  ...
  const addItemToRound = async(tid, mi, stockItems=[]) => {
    // Leer el estado más reciente de Firebase directamente
    // para evitar problemas con estado stale de React
    ...
    const currentItems = rounds[rIdx].items || [];
    // Si es un producto simple (sin stockItems especiales), acumular qty en item existente
    const isSimple = stockItems.length===0 || (stockItems.length===1 && stockItems[0].name===mi.name);
    const existing = isSimple ? currentItems.find(i=>i.menuId===mi.id && !i.paid) : null;
    ...
    rounds[rIdx]={...rounds[rIdx],items:newItems};
    saveTable(tid,{...t,rounds});
  };

  // ── Cerrar ronda — lee de Firebase, marca cerrada PRIMERO, luego descuenta ──
  const closeRound = async(tid) => {
    ...
    if(lastRound.closed===true) return;
    ...
    // PRIMERO marcar cerrada en Firebase — evita doble descuento si se da click 2 veces
    rounds[lastIdx]={...lastRound,closed:true,closedAt:now()};
    await saveTable(tid,{...t,rounds});

    // LUEGO descontar stock — usando query directa a Firebase
    // No depende de inventoryMap para garantizar que siempre funcione
    if(toDiscount.length>0){
      const { collection: col, getDocs: gDocs, writeBatch: wBatch, increment: inc, updateDoc: updDoc } = await import("firebase/firestore");
      const invSnap = await gDocs(col(db,"inventory"));
      const batch = wBatch(db);
      for(const {name, delta} of toDiscount){
        const found = invSnap.docs.find(d=>d.data().name===name);
        if(found) batch.update(found.ref, {stock: inc(delta)});
      }
      await batch.commit();
    }
  };
