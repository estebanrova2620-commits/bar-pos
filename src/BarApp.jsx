...
import { useFirebaseData, descuentaStock } from "./useFirebase";
...
export default function BarApp() {
  ...
  const closeRound = async(tid) => {
    ...
    // LUEGO descontar stock usando descuentaStock global
    // Esta función siempre lee de Firebase directamente, sin depender de estado React
    if(toDiscount.length>0){
      const itemsParaDescontar = toDiscount.map(({name, delta}) => ({name, qty: -delta}));
      await descuentaStock(itemsParaDescontar);
    }
  };
  ...
  const editLastRoundItem = async(tid, iid, newItem) => {
    ...
    const itemsAjuste = stockAdjustments.map(({name, delta}) => ({name, qty: -delta}));
    await descuentaStock(itemsAjuste);
    await saveTable(tid,{...t,rounds});
  };
