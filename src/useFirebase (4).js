...
export function useFirebaseData() {
  ...
  useEffect(()=>{ 
    ...
    // ── Inventario ────────────────────────────────────────────
    // Espera 3 segundos antes de inicializar para asegurar que Firebase
    // haya respondido con los datos reales (evita falsos "vacíos")
    let invTimer = null;
    unsubs.push(onSnapshot(collection(db,"inventory"), snap=>{
      if(!snap.empty){
        // HAY datos — cargarlos y cancelar cualquier timer de inicialización
        invInitialized = true;
        ...
        // Colección aparece vacía — esperar 3s antes de inicializar
        // por si Firebase está cargando datos existentes
        if(!invTimer){
          invTimer = setTimeout(()=>{
            // Si después de 3s sigue vacío, entonces inicializar
            if(!invInitialized){
              invInitialized = true;
