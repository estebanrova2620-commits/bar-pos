...
// ─────────────────────────────────────────────────────────────
// descuentaStock: función global que siempre lee de Firebase
// No depende de ningún estado de React
// items: [{name, qty}]  (qty positivo = descontar)
// ─────────────────────────────────────────────────────────────
export async function descuentaStock(items) {
  if (!items || items.length === 0) return;
  // Consolidar: sumar cantidades del mismo producto
  const mapa = {};
  for (const {name, qty} of items) {
    mapa[name] = (mapa[name] || 0) + qty;
  }
  // Leer inventario completo de Firebase
  const snap = await getDocs(collection(db, "inventory"));
  const batch = writeBatch(db);
  let cambios = 0;
  for (const [name, qty] of Object.entries(mapa)) {
    const docFound = snap.docs.find(d => d.data().name === name);
    if (docFound) {
      batch.update(docFound.ref, { stock: increment(-qty) });
      cambios++;
    }
  }
...
export function useFirebaseData() {
  ...
  useEffect(()=>{
    const unsubs = [];
    // Contador de colecciones listas
    let readyCount = 0;
    const total = 6; // config, inventory, users, expenses, credits, dailyLog
    const checkReady = () => { readyCount++; if(readyCount >= total) setReady(true); };
    ...
    unsubs.push(onSnapshot(doc(db,"config","global"), snap=>{
      if(snap.exists()){
        ...
        // Primera vez: crear config inicial sin tocar nada más
    ...
    // ── Inventario ────────────────────────────────────────────
    // REGLA: Si hay datos → cargar. Si está vacío → esperar 5s antes de inicializar.
    // El timer evita que un "vacío temporal" de Firebase borre datos reales.
    let invTimer = null;
    let invLoaded = false;
    unsubs.push(onSnapshot(collection(db,"inventory"), snap=>{
      if (!snap.empty) {
        // Hay datos — cargar siempre, nunca inicializar
        if (invTimer) { clearTimeout(invTimer); invTimer = null; }
        invLoaded = true;
        setInventory(snap.docs.map(d=>d.data()));
        if (!invLoaded) checkReady(); // solo contar una vez
        checkReady();
      } else if (!invLoaded) {
        // Vacío — esperar antes de asumir que es primera vez
        if (!invTimer) {
          invTimer = setTimeout(async () => {
            // Verificar de nuevo consultando directamente
            const verify = await getDocs(collection(db,"inventory"));
            if (verify.empty) {
              // Realmente vacío → inicializar por primera y única vez
              invLoaded = true;
              const batch = writeBatch(db);
    ...
    unsubs.push(onSnapshot(collection(db,"users"), snap=>{
      if (!snap.empty) {
        if (usersTimer) { clearTimeout(usersTimer); usersTimer = null; }
        usersLoaded = true;
        ...
        if (!usersTimer) {
          usersTimer = setTimeout(async () => {
            const verify = await getDocs(collection(db,"users"));
            if (verify.empty) {
              usersLoaded = true;
              const batch = writeBatch(db);
  ...
  // adjustInv simple — para ajustes manuales desde inventario
