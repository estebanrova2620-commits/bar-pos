...
export function useFirebaseData() {
  ...
  // inventoryMap para búsqueda rápida por nombre sin re-query
  ...
  useEffect(()=>{
    const unsubs=[];
    let initialized = {inv:false, users:false, promo:false};
    ...
    unsubs.push(onSnapshot(collection(db,"inventory"), snap=>{
      if(snap.empty && !initialized.inv){
        ...
        setInventory(items);
        // Construir mapa nombre->id para adjustInv rápido
        const map = {};
    ...
    // ── Promociones/Cubetazos personalizados ──────────────────
  ...
  // ── Inventario — adjustInv ATÓMICO con increment ──────────────
  // Usa el inventoryMap (nombre->id) para evitar re-query a Firestore
  // increment() es atómico: múltiples llamadas simultáneas suman correctamente
  const adjustInv = useCallback(async(name, delta)=>{
    const itemId = inventoryMap[name];
    if(!itemId){
      // Fallback: buscar por query si el map todavía no tiene el item
  ...
  // adjustInvBatch: descuenta múltiples items de una vez de forma atómica
  // items: [{name, delta}]
  const adjustInvBatch = useCallback(async(items)=>{
    ...
    for(const {name, delta} of items){
      ...
      if(itemId){
        ...
        // Fallback individual si no está en el map
        const snap = await getDocs(collection(db,"inventory"));
        const found = snap.docs.find(d=>d.data().name===name);
  ...
  const savePromocion = useCallback(async(promo)=>{
    ...
    // Actualizar estado activo en config global
    setDoc(doc(db,"config","global"),{
      cubetazos:{...cubetazos,[promo.id]:promo.activo}
    },{merge:true});
  },[cubetazos]);

  const deletePromocion = useCallback(async(id)=>{
    await deleteDoc(doc(db,"promociones",id));
    // Remover del config global
    const next={...cubetazos};
    delete next[id];
    setDoc(doc(db,"config","global"),{cubetazos:next},{merge:true});
  },[cubetazos]);
