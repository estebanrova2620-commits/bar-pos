...
import {
  ...
  deleteDoc, writeBatch, getDocs, increment, getDoc
...
export function useFirebaseData() {
  ...
  useEffect(()=>{
    ...
    unsubs.push(onSnapshot(doc(db,"config","global"), snap=>{
      if(snap.exists()){
        ...
        setDayOpState(null);
        // Primera vez: guardar categorías iniciales
    ...
    // ── Inventario ────────────────────────────────────────────
    // CRÍTICO: Solo inicializa si el doc "meta/inventory" no existe
    // Esto garantiza que los datos reales nunca se sobreescriban
    unsubs.push(onSnapshot(collection(db,"inventory"), async snap=>{
      if(snap.empty){
        // Verificar si ya fue inicializado antes con un doc de control
        const metaDoc = await getDoc(doc(db,"meta","inventory"));
        if(!metaDoc.exists()){
          // Primera vez genuina — inicializar
          const batch=writeBatch(db);
          INIT_INV.forEach(item=>batch.set(doc(db,"inventory",item.id),item));
          // Marcar como inicializado para nunca volver a sobreescribir
          batch.set(doc(db,"meta","inventory"),{initialized:true, at: new Date().toISOString()});
          await batch.commit();
        }
        // Si metaDoc existe pero inventory está vacío, algo raro pasó — no hacer nada
      } else {
    ...
    // ── Usuarios ──────────────────────────────────────────────
    // Igual que inventario — solo inicializa una vez
    unsubs.push(onSnapshot(collection(db,"users"), async snap=>{
      if(snap.empty){
        const metaDoc = await getDoc(doc(db,"meta","users"));
        if(!metaDoc.exists()){
          const batch=writeBatch(db);
          INIT_USERS.forEach(u=>batch.set(doc(db,"users",u.id),u));
          batch.set(doc(db,"meta","users"),{initialized:true, at: new Date().toISOString()});
          await batch.commit();
        }
      } else {
    ...
    // ── Auditoría — NUNCA se borra, solo se acumula ───────────
  ...
  const adjustInv = useCallback(async(name, delta)=>{
    ...
    // Fallback si el map aún no tiene el item
    const snap = await getDocs(collection(db,"inventory"));
    const found = snap.docs.find(d=>d.data().name===name);
    if(found){
      await updateDoc(doc(db,"inventory",found.id),{ stock: increment(delta) });
    }
  },[inventoryMap]);

  // adjustInvBatch — múltiples items en una sola operación atómica
  const adjustInvBatch = useCallback(async(items)=>{
    ...
    for(const {name, delta} of items){
      ...
      if(itemId){
        ...
        missingNames.push({name, delta});
      }
    }

    // Resolver los que no estaban en el map
    if(missingNames.length > 0){
      const snap = await getDocs(collection(db,"inventory"));
      for(const {name, delta} of missingNames){
        const found = snap.docs.find(d=>d.data().name===name);
  ...
  const saveInventoryItem = useCallback(async(item)=>{
    ...
    // Actualizar meta para confirmar que existe
    setDoc(doc(db,"meta","inventory"),{initialized:true},{merge:true});
  },[]);
  ...
  const saveUser = useCallback(async(user)=>{
    ...
    // Actualizar meta
    setDoc(doc(db,"meta","users"),{initialized:true},{merge:true});
  },[]);
  ...
  // ── Auditoría — NUNCA se borra externamente ───────────────────
